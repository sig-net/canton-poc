...
const toHex = (hex: string): Hex => `0x${hex}`;
...
export function buildTxRequest(p: CantonEvmType2Params): Eip1559TxFields {
  return {
    ...
    to: p.to === null ? null : toHex(p.to),
    value: toBig(p.value),
    data: p.calldata === "" ? "0x" : toHex(p.calldata),
    accessList: p.accessList.map((e) => ({
      address: toHex(e.address),
      storageKeys: e.storageKeys.map(toHex),
    })),
  };
}
...
export function reconstructSignedTx(
  ...
  return serializeTransaction(
    buildTxRequest(p) as unknown as TransactionSerializableEIP1559,
    { r: signature.r, s: signature.s, yParity: signature.v },
  );
}
