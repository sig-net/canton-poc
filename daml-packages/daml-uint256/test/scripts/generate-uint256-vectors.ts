...
function buildDamlFile(fuzz: FuzzVector[]): string {
  ...
  lines.push(`import DA.Text (take)`);
  ...
  lines.push(``);
  lines.push(
    `fuzzVectors : [(BytesHex, BytesHex, BytesHex, BytesHex, BytesHex, Int, BytesHex, Int)]`,
  );
  ...
  for (const [i, v] of fuzz.entries()) {
    const prefix = i === 0 ? "  [ " : "  , ";
    lines.push(
      `${prefix}("${v.a}", "${v.b}", "${v.add}", "${v.sub}", "${v.mul}", ${v.d}, "${v.divQ}", ${v.divR})`,
    );
  }
  lines.push(`  ]`);
  lines.push(``);
  lines.push(`testFuzzAdd : Script ()`);
  lines.push(`testFuzzAdd = forA_ fuzzVectors $ \\(a, b, addExp, _, _, _, _, _) ->`);
  lines.push(`  assertMsg ("fuzz add " <> take 8 a) (hexAddUint256 a b == addExp)`);
  lines.push(``);
  lines.push(`testFuzzSub : Script ()`);
  lines.push(`testFuzzSub = forA_ fuzzVectors $ \\(a, b, _, subExp, _, _, _, _) ->`);
  lines.push(`  assertMsg ("fuzz sub " <> take 8 a) (hexSubUint256 a b == subExp)`);
  lines.push(``);
  lines.push(`testFuzzMul : Script ()`);
  lines.push(`testFuzzMul = forA_ fuzzVectors $ \\(a, b, _, _, mulExp, _, _, _) ->`);
  lines.push(`  assertMsg ("fuzz mul " <> take 8 a) (hexMulUint256 a b == mulExp)`);
  lines.push(``);
  lines.push(`testFuzzDiv : Script ()`);
  lines.push(`testFuzzDiv = forA_ fuzzVectors $ \\(a, _, _, _, _, d, divQExp, divRExp) -> do`);
  lines.push(`  let (q, r) = uint256DivInt (uint256FromHex a) d`);
  lines.push(`  assertMsg ("fuzz divQ " <> take 8 a) (uint256ToHex q == divQExp)`);
  lines.push(`  assertMsg ("fuzz divR " <> take 8 a) (r == divRExp)`);
